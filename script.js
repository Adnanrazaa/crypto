  ...
  fetch(apiUrl)
    ...
    .then((data) => {
      const filteredData = data.filter((crypto) =>
        specificCoins.includes(crypto.symbol)
      );
      let cryptosToDisplay;

      if (activeTab === "gainers") {
        cryptosToDisplay = filteredData
          .filter(
            (crypto) =>
              crypto.performance && crypto.performance[selectedTimeFrame] >= 0.1
          )
          .sort(
            (a, b) =>
              b.performance[selectedTimeFrame] -
              a.performance[selectedTimeFrame]
          );
      } else {
        cryptosToDisplay = filteredData
          .filter(
            (crypto) =>
              crypto.performance &&
              crypto.performance[selectedTimeFrame] <= -0.1
          )
          .sort(
            (a, b) =>
              a.performance[selectedTimeFrame] -
...
function displayCryptos(cryptos) {
  ...
  cryptos.forEach((crypto) => {
    ...
    const tradingViewLink = `https://www.tradingview.com/chart/?symbol=${crypto.symbol.toUpperCase()}`;
    ...
    listItem.innerHTML = `
            <div class="ms-2 me-auto">
                <div class="fw-bold">
                    <img style="margin-right: 10px;" src="https://cryptobubbles.net/backend/${
                      crypto.image
                    }" alt="${crypto.name}" width="30" height="30">
            ...
            <span class="badge text-bg-primary rounded-pill fs-5">${
              crypto.performance[selectedTimeFrame]
            }%</span>
                        <a href="${tradingViewLink}" target="_blank" class="btn btn-primary ms-3">View on TradingView</a>
